...
static void celix_frameworkLogger_vlogInternal(celix_framework_logger_t* logger, celix_log_level_e level, celix_status_t *optionalStatus, const char* file, const char* function, int line, const char* format, va_list args) {
...
void celix_frameworkLogger_vlog(celix_framework_logger_t* logger, celix_log_level_e level, const char *func, const char *file, int line,
                          const char *format, va_list formatArgs) {
    celix_frameworkLogger_vlogInternal(logger, level, NULL, func, file, line, format, formatArgs);
}


void celix_frameworkLogger_log(celix_framework_logger_t*  logger, celix_log_level_e level, const char *func, const char *file, int line, const char *format, ...) {
    va_list args;
    va_start(args, format);
    celix_frameworkLogger_vlogInternal(logger, level, NULL, file, func, line, format, args);
    va_end(args);
}

void celix_frameworkLogger_logCode(celix_framework_logger_t*  logger, celix_log_level_e level, const char *func, const char *file, int line, celix_status_t code, const char *format, ...) {
    va_list args;
    va_start(args, format);
    celix_frameworkLogger_vlogInternal(logger, level, &code, file, func, line, format, args);
    va_end(args);
}