...
celix_status_t bundleCache_create(const char *fwUUID, celix_properties_t *configurationMap, bundle_cache_pt *bundle_cache) {
	...
	CELIX_FRAMEWORKLOGGER_LOG_IF_ERROR(celix_frameworkLogger_globalLogger(), status, NULL, "Failed to create bundle cache");
...
celix_status_t bundleCache_getArchives(bundle_cache_pt cache, array_list_pt *archives) {
	...
	if (dir != NULL) {
		...
		if (errno != 0) {
			CELIX_FRAMEWORKLOGGER_LOG(celix_frameworkLogger_globalLogger(), CELIX_LOG_LEVEL_ERROR, "Error reading dir");
	...
	CELIX_FRAMEWORKLOGGER_LOG_IF_ERROR(celix_frameworkLogger_globalLogger(), status, NULL, "Failed to get bundle archives");
	if (status != CELIX_SUCCESS) {
		perror("\t");
	}
...
celix_status_t bundleCache_createArchive(bundle_cache_pt cache, long id, const char * location, const char *inputFile, bundle_archive_pt *bundle_archive) {
	...
	CELIX_FRAMEWORKLOGGER_LOG_IF_ERROR(celix_frameworkLogger_globalLogger(), status, NULL, "Failed to create archive");
...
static celix_status_t bundleCache_deleteTree(bundle_cache_pt cache, char * directory, bool root) {
    ...
    if (!root) {
        ...
        CELIX_FRAMEWORKLOGGER_LOG_IF_ERROR(celix_frameworkLogger_globalLogger(), status, NULL, "Failed to delete tree at dir '%s'. %s", directory, err);
    }
