...
celix_status_t manifest_create(manifest_pt *manifest) {
	...
	*manifest = malloc(sizeof(**manifest));
    ...
    CELIX_FRAMEWORKLOGGER_LOG_IF_ERROR(celix_frameworkLogger_globalLogger(), status, NULL, "Cannot create manifest");
...
celix_status_t manifest_createFromFile(const char *filename, manifest_pt *manifest) {
    ...
    CELIX_FRAMEWORKLOGGER_LOG_IF_ERROR(celix_frameworkLogger_globalLogger(), status, NULL, "Cannot create manifest from file");
	...
	if (file != NULL) {
		...
		while (status==CELIX_SUCCESS && fgets(lbuf, sizeof(lbuf), file) != NULL) {
			...
			if (lbuf[--len] != '\n') {
                ...
                CELIX_FRAMEWORKLOGGER_LOG_IF_ERROR(celix_frameworkLogger_globalLogger(), status, NULL, "Manifest '%s' line too long", filename);
				break;
			}
			...
			if (strlen(name) == 0) {
				...
				if ((tolower(lbuf[0]) == 'n') && (tolower(lbuf[1]) == 'a') &&
					...
					status = CELIX_FILE_IO_EXCEPTION;
                    CELIX_FRAMEWORKLOGGER_LOG_IF_ERROR(celix_frameworkLogger_globalLogger(), status, NULL, "Manifest '%s' invalid format", filename);
					break;
				}
...
//					lastline = realloc(lastline, strlen(buf) + 1);
    ...
    CELIX_FRAMEWORKLOGGER_LOG_IF_ERROR(celix_frameworkLogger_globalLogger(), status, NULL, "Cannot read manifest");
