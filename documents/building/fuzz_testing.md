...
The Celix Fuzzing workflow uploads the generated corpora, logs, and any
`crash-*` reproducer files as a build artifact named `fuzzing-corpora-artifact`. 
These artifacts to keep the seed corpus in `libs/utils/fuzzing/{filter,properties,version}_corpus` updated. 
